...
Toast.makeText = function (text, duration) {
  ...
  let _x = 0
  let _y = 0
  const ToastObject = function (tText, tDuration) {
    _text = tText
    _duration = tDuration
    //
    ...
    this.show = function () {
      ...
      // 背景图片与文本内容的间距
      const hPadding = textLabel.fontSize / 8
      const vPadding = 2
      ...
      const action = cc.sequence(cc.moveBy(_duration, cc.v2(0, 0)), cc.fadeOut(0.3), finished)
